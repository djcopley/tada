...
import { runVisual } from '../../src/design/status'
...
function TicketDetailBody({ ticketId, ticket, startEditing = false }: { ticketId: number; ticket: ApiTicketDetail; startEditing?: boolean }) {
  ...
  const stopped = run?.status === 'held' || run?.status === 'failed'
  ...
  // The idle/live state strip under the brief — everything that isn't a hold.
  ...
  if (!run) {
    stateStrip = (
      <View style={styles.stateBlock}>
        ...
        <View style={styles.row}>
          <Button testID="ticket-queue" small label="Queue" loading={moveTicket.isPending} onPress={() => moveTicket.mutate({ id: ticketId, to: { column: 'queued' } })} />
          <View style={styles.spacer} />
    ...
    stateStrip = (
      <View style={styles.row}>
        ...
        <View style={styles.spacer} />
        <Button testID="ticket-to-backlog" variant="ghost" small label="Move to backlog" onPress={() => moveTicket.mutate({ id: ticketId, to: { column: 'backlog' } })} />
      </View>
    )
  ...
  } else if (run.status === 'done' || ticket.column === 'done') {
    stateStrip = (
      <View style={styles.row}>
        <RunStatusChip status="ok" label="done" meta={run.finishedAt ? `moved itself ${hhmm(run.finishedAt)}` : undefined} />
        <View style={styles.spacer} />
        {canUndoDone(ticket, now) ? (
          <Button testID="ticket-undo-done" variant="ghost" small label="Undo" onPress={() => moveTicket.mutate({ id: ticketId, to: { column: 'backlog' } })} />
        ) : null}
      </View>
    )
